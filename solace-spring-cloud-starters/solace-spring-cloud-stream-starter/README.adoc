...
==== Solace Session Health-Check Properties

These properties configure the Solace session's health indicator configurable under `solace.health-check.session`.

reconnectAttemptsUntilDown::
The number of session reconnect attempts until the health goes `DOWN`. This will happen regardless if the underlying session is actually still reconnecting. Set this to `0` will disable this feature.
+
