...
public class JCSMPMessageSource extends AbstractMessageSource<Object> implements Lifecycle, Pausable {
    ...
    protected Object doReceive() {
        ...
        try {
            if (remoteStopFlag.get()) {
                if (log.isDebugEnabled()) {
                    log.debug(String.format("Message source %s is not running. Cannot receive message", id));
                }
                return null;
            } else if (!isRunning()) {
                ...
                log.warn("{}{}", closedBindingException, msg0);
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                ...
                // Hide this so we don't flood the log.
                log.debug(String.format("Unable to receive message from endpoint %s", consumerDestination.getName()), e);
            }
            return null;
        }
    ...
    private Message<?> processMessage(MessageContainer messageContainer) {
        ...
        try {
            ...
            log.warn(String.format("XMLMessage %s cannot be consumed. It will be requeued",
    ...
    public void start() {
        ...
        writeLock.lock();
        try {
            log.info(String.format("Creating consumer to %s %s <message source ID: %s>",
                    consumerProperties.getExtension().getEndpointType(), consumerDestination.getName(), id));
            if (isRunning()) {
                log.warn(String.format("Nothing to do, message source %s is already running", id));
                return;
            }
    ...
    public void stop() {
        ...
        try {
            if (!isRunning()) return;
            log.info(String.format("Stopping consumer to endpoint %s <message source ID: %s>", consumerDestination.getName(), id));
    ...
    public void pause() {
        ...
        writeLock.lock();
        try {
            log.info(String.format("Pausing message source %s", id));
            if (flowReceiverContainer != null) {
                flowReceiverContainer.pause();
            }
            paused = true;
    ...
    public void resume() {
        ...
        writeLock.lock();
        try {
            log.info(String.format("Resuming message source %s", id));
            if (flowReceiverContainer != null) {
                try {
